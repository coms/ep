...
public class Problem55 {
	...
	public static void main(String[] args) {
		...
		for (long n = 1; n < 10_000; n++) {
			//boolean lychrel = DigitUtil.isPalindromic(n);
			boolean lychrel = false;
			int iterations = 0;
			BigInteger t = BigInteger.valueOf(n);
			while (!lychrel && iterations < 51) {
				t = t.add(DigitUtil.reverse(t));
				iterations++;
				lychrel = DigitUtil.isPalindromic(t.toString());
			}
			System.out.println(n + " : " + !lychrel + " : " + iterations);
			if(!lychrel) {
				sum++;
			}
		}
